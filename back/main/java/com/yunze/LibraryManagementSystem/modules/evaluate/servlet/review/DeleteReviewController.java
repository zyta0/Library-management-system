 ...
 * 删除回复
 */
@WebServlet(name = "deleteReviewController", value = "/safe/deleteReviewController")
public class DeleteReviewController extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        int reviewId = (int) jsonMap.get("reviewId");
        //处理数据
        ReviewService reviewService = new ReviewServiceImpl();
        Review review = reviewService.select(reviewId);
        int result1 = reviewService.remove(review);
        Map<String, Object> responseMap = new HashMap<>();
        if (review == null) {
            responseMap.put("status", "failure");
            responseMap.put("code", 1000);
            responseMap.put("massage", "错误参数，回复不存在");
        }else {
            //书评的评论量-1
            EvaluateService evaluateService = new EvaluateServiceImpl();
            Evaluate evaluate = evaluateService.search(review.getEvaluateId());
            if (evaluate == null) {
                responseMap.put("status", "failure");
                responseMap.put("code", 1000);
